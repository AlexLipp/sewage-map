"""Standardise Wessex Water EDM discharge-time worksheets.

Only the explicitly named ``Discharge Times`` sheets contain event rows.
README and Summary sheets are structural and are deliberately not processed.
"""
...
RAW_FOLDER = ROOT / "raw_data" / "wessex"
OUTPUT_FOLDER = ROOT / "clean_EIR_stopstartdata" / "input_stopstart_data"
OUTPUT_FILE = OUTPUT_FOLDER / "wessex_cleaned_data.csv"
REJECTED_FILE = OUTPUT_FOLDER / "rejected_rows" / "wessex_rejected_rows.csv"
DURATION_QC_FILE = OUTPUT_FOLDER / "duration_qc" / "wessex_duration_discrepancies.csv"

SOURCE_WORKBOOKS = [
    "storm-overflow-event-duration-monitoring-data-2025 (1).xlsx",
    "storm-overflow-event-duration-monitoring-data-2026-ytdxlsx.xlsx",
]
EVENT_SHEET = "Discharge Times"
STRUCTURAL_SHEETS = ["README", "Summary"]
SOURCE_COLUMNS = [
    "Unique ID", "SO Name", "Discharge Start", "Discharge End", "Total Duration (Hrs)"
]
OUTPUT_COLUMNS = [
...
def require_columns(data: pd.DataFrame, source_file: str) -> None:
    missing_columns = []
    ...
    if missing_columns:
        raise KeyError(
            f"Expected columns are missing from {source_file} [{EVENT_SHEET}]: "
            f"{missing_columns}. The supplier schema may have changed."
        )


def clean_workbook(filename: str) -> tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame, dict]:
    print(f"[{LABEL}] Reading: {filename} [{EVENT_SHEET}]")
    # Header row 1 and this exact worksheet are verified for both releases.
    data = pd.read_excel(
        RAW_FOLDER / filename, sheet_name=EVENT_SHEET, header=0, dtype=object
    )
    rows_loaded = len(data)
    require_columns(data, filename)
    selected = data[SOURCE_COLUMNS].copy()
    selected["source_row_number"] = range(2, rows_loaded + 2)
    selected = selected.rename(columns={
        "Unique ID": "permit_original", "SO Name": "location_original",
        "Discharge Start": "start_original", "Discharge End": "stop_original",
        "Total Duration (Hrs)": "supplier_duration_hours",
    })
    ...
    for index in selected.index[rejected_mask]:
        originals = {
            ...
            "SO Name": selected.at[index, "location_original"],
            "Discharge Start": selected.at[index, "start_original"],
            "Discharge End": selected.at[index, "stop_original"],
            "Total Duration (Hrs)": selected.at[index, "supplier_duration_hours"],
        }
        rejected_rows.append({
            "company": "wessex", "source_file": f"raw_data/wessex/{filename}",
            "source_sheet": EVENT_SHEET,
    ...
    # Supplier duration is in hours and is used only for QC.  Website duration
    # is always the timestamp-derived value above.
    ...
    duration_qc = pd.DataFrame({
        "company": "wessex",
        "source_file": f"raw_data/wessex/{filename}",
        "source_sheet": EVENT_SHEET,
...
def validate_output(data: pd.DataFrame) -> None:
    if list(data.columns) != OUTPUT_COLUMNS or data.empty:
        raise ValueError("Wessex output does not have the exact five-column contract.")
    ...
    if start.isna().any() or stop.isna().any() or stop.lt(start).any():
        raise ValueError("Wessex output contains an invalid timestamp pair.")
    if duration.isna().any() or duration.lt(0).any() or not duration.map(math.isfinite).all():
        raise ValueError("Wessex output contains an invalid duration.")
    expected = (stop - start).dt.total_seconds() / 60
    if duration.sub(expected).abs().gt((1 / 60) + 1e-9).any():
        raise ValueError("Wessex duration differs from timestamps by over one second.")


def clean_wessex(*, dry_run: bool = False, strict: bool = False) -> int:
    cleaned_frames = []; rejected_frames = []; qc_frames = []; summaries = []
    for filename in SOURCE_WORKBOOKS:
        cleaned, rejected, duration_qc, summary = clean_workbook(filename)
        cleaned_frames.append(cleaned); rejected_frames.append(rejected)
        qc_frames.append(duration_qc); summaries.append(summary)
    ...
    duration_qc = pd.concat(qc_frames, ignore_index=True)
    if not dry_run:
    ...
    print(f"[{LABEL}] Files processed: {len(SOURCE_WORKBOOKS)}")
...
def main() -> int:
    ...
    if arguments.self_check:
        ...
        assert clean_permits(pd.Series(["0012"])).iloc[0] == "0012"
        print(f"[{LABEL}] Self-check passed")
        return 0
    return clean_wessex(dry_run=arguments.dry_run, strict=arguments.strict)
